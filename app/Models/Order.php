...
class Order extends Model
{
    ...
    /**
     * Zyda scraped order associated with this order (if source = 'zyda')
     */
    public function zydaOrder(): HasOne
    {
        return $this->hasOne(ZydaOrder::class);
    }
            ...
            if (!empty($order->shop_id) && empty($order->dsp_order_id)) {
                try {
                    ...
                    if ($shippingResult && isset($shippingResult['dsp_order_id'])) {
                        ...
                        $order->shipping_provider = $shippingResult['shipping_provider'] ?? $provider; // Update provider
                        $order->save();
