    ...
    <AppLayout :breadcrumbs="breadcrumbs">
                ...
                >
                    <div class="h-1.5 w-full transition-all duration-300"
                        :class="showTable
                            ...
                            : 'bg-gradient-to-r from-gray-200 to-gray-300 group-hover:from-emerald-300 group-hover:to-teal-400 dark:from-gray-600 dark:to-gray-500'"
                    />
                    <div class="p-5">
                    ...
                    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400 dark:text-gray-500">اضغط على أي سطر لعرض تفاصيل الفاتورة</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            ...
                            <a
                                ...
                                class="inline-flex items-center gap-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 px-4 py-2 text-xs font-semibold text-white shadow-sm transition-colors duration-150"
        ...
        <Teleport to="body">
            ...
            >
                <div
                    v-if="drawer"
                    class="fixed top-0 left-0 z-50 h-full w-full max-w-md bg-white dark:bg-gray-900 shadow-2xl overflow-y-auto"
                    dir="rtl"
                >
                    <!-- Header -->
                    ...
                    <div class="p-5 space-y-5">
                        ...
                        <div :class="['flex items-center gap-2.5 rounded-xl border px-4 py-3',
                            ...
                            <span v-if="drawer.collected_at" class="mr-auto text-xs text-emerald-600 dark:text-emerald-400">
                                {{ drawer.collected_at }}
                            </span>
                        </div>
                        ...
                        <div class="rounded-xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 p-4 space-y-2.5">
                            ...
                            <div class="flex items-center gap-2.5 text-sm text-gray-700 dark:text-gray-200">
                                <User class="h-4 w-4 text-gray-400 flex-shrink-0" />
                                {{ drawer.customer_name }}
                            </div>
                            <div v-if="drawer.customer_phone" class="flex items-center gap-2.5 text-sm text-gray-700 dark:text-gray-200">
                                <Phone class="h-4 w-4 text-gray-400 flex-shrink-0" />
                                {{ drawer.customer_phone }}
                            </div>
                            <div v-if="drawer.customer_address" class="flex items-start gap-2.5 text-sm text-gray-700 dark:text-gray-200">
                                <MapPin class="h-4 w-4 text-gray-400 flex-shrink-0 mt-0.5" />
                                {{ drawer.customer_address }}
                            </div>
                        </div>
                        ...
                        <div class="rounded-xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 p-4 space-y-2.5">
                            ...
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span :class="['rounded-full border px-2.5 py-0.5 text-xs font-medium', paymentColor(drawer.payment_method)]">
                                    {{ paymentLabel(drawer.payment_method) }}
                                </span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <span class="flex items-center gap-2 text-gray-500 dark:text-gray-400"><Package class="h-4 w-4" /> حالة الطلب</span>
                                <span :class="['rounded-full border px-2.5 py-0.5 text-xs font-medium', statusColor(drawer.order_status)]">
                                    {{ statusLabel(drawer.order_status) }}
                                </span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                        ...
                        <div v-if="drawer.items.length" class="rounded-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                            ...
                            <div class="divide-y divide-gray-50 dark:divide-gray-700/60 bg-white dark:bg-gray-800">
                                <div v-for="(item, i) in drawer.items" :key="i"
                                    class="flex items-center justify-between px-4 py-3 text-sm">
                                    <div class="flex items-center gap-2.5">
                                        <span class="flex h-6 w-6 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700 text-xs font-bold text-gray-500 dark:text-gray-300">
                                            {{ item.quantity }}
                                        </span>
                        ...
                        <div class="rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden">
                            <div class="divide-y divide-gray-50 dark:divide-gray-700/60 px-4">
                                <div class="flex justify-between py-3 text-sm text-gray-600 dark:text-gray-300">
                                    <span>قيمة المنتجات</span>
                                    <span>{{ fmt(drawer.subtotal) }}</span>
                                </div>
                                <div class="flex justify-between py-3 text-sm text-gray-600 dark:text-gray-300">
                                    <span>رسوم التوصيل</span>
                                    <span>{{ fmt(drawer.delivery_fee) }}</span>
                                </div>
                                <div class="flex justify-between py-3 text-sm text-gray-600 dark:text-gray-300">
                                    <span>الضريبة</span>
                                    <span>{{ fmt(drawer.tax) }}</span>
                                </div>
                                <div class="flex justify-between py-3 text-base font-bold text-emerald-600 dark:text-emerald-400">
                                    <span>الإجمالي</span>
                                    <span>{{ fmt(drawer.total) }}</span>
                                </div>
                            </div>
                        </div>
